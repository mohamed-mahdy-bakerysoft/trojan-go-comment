...
# 开启 websocket 传输，使用TLS+Websocket承载Trojan协议
open_websocket(){
...
install_bbr() {
  wget -N --no-check-certificate "https://raw.githubusercontent.com/lzh06550107/trojan-go-comment/master/install/tcp.sh"
  chmod +x tcp.sh
  ./tcp.sh
}
...
download_trojan_mgr(){
  ...
  curl -s -o /etc/trojan_mgr.sh https://raw.githubusercontent.com/lzh06550107/trojan-go-comment/master/install/trojan_mgr.sh
...
trojan_go_info_html() {
  vps="Trojan-go"
  wget --no-check-certificate -O ${web_dir}/trojan_go_tmpl.html https://raw.githubusercontent.com/lzh06550107/trojan-go-comment/master/install/trojan_go_tmpl.html
